  ...
  return (
    <div className="p-10 bg-white rounded-[2.5rem] border border-slate-200"> 
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {medicines.filter(m => m.name.toLowerCase().includes(search.toLowerCase())).map((medicine) => ( 

          <div key={med.id} className="p-5 rounded-2xl bg-slate-50 border relative">
            
            <p className="font-bold text-slate-800">{medicine.name}</p>

            <p className={`text-lg font-black ${medicine.stock < 50 ? "text-red-500" : "text-blue-600"}`}>{med.stock}u</p>
            <button onClick={() => deleteMedicine(medicine.id)} className="absolute top-4 right-4 text-slate-400 

            hover:text-red-500 font-bold">Remove</button> 
          </div>
        ))}
      </div>
    </div>
