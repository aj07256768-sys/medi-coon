...
const PharmacyStock = ({ medicines = [], setMedicines }) => {
  ...
  // Saves permanently to db.json (Create)
  ...
  // Deletes permanently from db.json (Delete)
  ...
  return (
    <div className="p-10 bg-white rounded-[2.5rem] border border-slate-200"> 
      <h2 className="text-xl font-black uppercase mb-6">Pharmacy Inventory</h2>

      <input 
        value={search} 
        onChange={(e) => setSearch(e.target.value)} 
        placeholder="Search medicine..." 
        className="w-full p-4 rounded-2xl border mb-6 bg-slate-50 font-bold text-sm" 
      /> 
      ...
      <div className="flex gap-4 mb-9">   
        <input 
          ...
          placeholder="Name" 
          className="p-3 border rounded-xl text-sm font-bold bg-slate-50" 
        />
        <input 
          value={newMed.stock} 
          ...
          className="p-3 border rounded-xl text-sm font-bold bg-slate-50" 
        />
        <button onClick={addMedicine} className="bg-blue-600 text-white px-5 rounded-xl text-xs font-black uppercase">
          Add
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {medicines
          ...
          .map((medicine) => ( 
            <div key={medicine.id} className="p-5 rounded-2xl bg-slate-50 border relative">
              <p className="font-bold text-slate-800">{medicine.name}</p>
              <p className={`text-lg font-black ${medicine.stock < 50 ? "text-red-500" : "text-blue-600"}`}>
                {medicine.stock}u
              </p>
              <button 
